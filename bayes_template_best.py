...
class Bayes_Classifier_Best:
   ...
   def __init__(self, trainDirectory = ("reviews" + "/")):
      ...
      obj = self.load("dictionaries_best")
      if obj == None:
         iFileList = []
         ...
         for filename in iFileList:
            ...
            unigramWords = self.tokenize(stringWords)
            bigramWords = self.bigram(unigramWords)

            tokenizeWords = unigramWords + bigramWords
            if (result == "positive"):
   ...
   def classify(self, sText):
      ...
      '''
      unigram_words = self.tokenize(sText)
      words = self.bigram(unigram_words)
      P = []
   ...
   def smoothing_positive(self, words):
       ...
       # probability calculated differently 
       # P(w(i) | w(i-1)) = c(w(i-1), w(i)) / c(w(i))
      P_positive = math.log( ( (self.results["positive"]) / (sum(self.results.values())) ))
      for word in words:
         if word in self.positiveWords:
            P_positive += (math.log( ( (self.positiveWords[word]) / (self.positiveWords[word[0]]) ) ))
         else:
            # 1-Smoothing
            ...
            if word[0] not in self.positiveWords:
                self.positiveWords[word[0]] = 1
            if word[1] not in self.positiveWords:
                self.positiveWords[word[1]] = 1
   ...
   def smoothing_negative(self, words):
      ...
      for word in words:
         if word in self.negativeWords:
            P_negative += (math.log( ( (self.negativeWords[word]) / (self.negativeWords[word[0]]) ) ))
         else:
            # 1-Smoothing
            ...
            if word[0] not in self.negativeWords:
                self.negativeWords[word[0]] = 1
            if word[1] not in self.negativeWords:
                self.negativeWords[word[1]] = 1
   ...
   def bigram(self, tokenizeWords):
      newTokenizeWords = []

      for i in range(0, (len(tokenizeWords)-1)):
        newTokenizeWords.append((tokenizeWords[i], tokenizeWords[i+1]))

      return newTokenizeWords  
